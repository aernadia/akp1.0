...
class ReportController extends Controller
{
    ...
    private function getHiredEmployees($constraints) {
        $employees = Employee::where('pengesahanpertama', '>=', $constraints['from'])
                        ->where('pengesahanpertama', '<=', $constraints['to'])
                        ->get();
        return $employees;
    }
    ...
    private function getExportingData($constraints) {
        ...
        'employees.age','employees.lantikanpertama', 'employees.address', 'employees.zip', 'employees.pengesahanpertama',
        'department.name as department_name', 'division.name as division_name')
        ->where('pengesahanpertama', '>=', $constraints['from'])
        ->where('pengesahanpertama', '<=', $constraints['to'])
        ->get()
